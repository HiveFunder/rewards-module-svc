...
const fakedata = fs.createWriteStream('fakedata.csv');
let tiers;
let check;
  ...
  // arbitrary trap to semi-randomize reward tier levels for different projects
    if (i % 8 === 0) {
	  ...
	  for (let j = 0; j < tiers.length; j += 1) {
	    ...
	    check = fakedata.write(`${random.projectId},${random.pledgeAmount},${random.name},${random.description},${random.item1},${random.item2},${random.item3},${random.isLimited},${random.limitCount},${random.estDeliv},${random.shipsTo},${random.backers}` + "\n");
	  }
